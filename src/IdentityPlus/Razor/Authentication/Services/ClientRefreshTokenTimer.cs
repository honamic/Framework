...
public class ClientRefreshTokenTimer : IClientRefreshTokenTimer
{
    ...
    private async Task<TimeSpan?> GetTimerIntervalAsync()
    {
        ...
        var diff = token.CreateOn?.Add(TimeSpan.FromSeconds(token.ExpiresIn)) - DateTimeOffset.Now;

        if (diff < TimeSpan.FromSeconds(7))
        {
            _logger.LogInformation("BearerToken was expired");
            return null;
        }

        return diff - TimeSpan.FromSeconds(7);
    }
